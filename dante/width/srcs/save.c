...
t_map	*saver(t_map *map, int height, int width, int dir)
{
  if (check_nb_posr(map, height, width) > 1)
  {
    map->my_signal = 1;
    map->save[map->cmp_save]->x = map->x;
    map->save[map->cmp_save++]->y = map->y;
  }
  else if (check_nb_posr(map, height, width) == 0)
  {
    map->my_signal = 2;
    map->x = map->save[map->cmp_save - 1]->x;
    map->y = map->save[map->cmp_save - 1]->y;
  }
  if (check_nb_posr(map, height, width) == 0)
  {
    while (check_nb_posr(map, height, width) == 0)
	  {
	    map->cmp_save--;
	    map->x = map->save[map->cmp_save - 1]->x;
	    map->y = map->save[map->cmp_save - 1]->y;
